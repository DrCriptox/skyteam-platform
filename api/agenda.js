export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      await fetch(BIN_URL, { method: 'PUT', headers: HEADERS, body: JSON.stringify(record) });
      ...
      if (action === 'saveBooking' && booking && process.env.RESEND_API_KEY) {
        ...
        if (socioEmail) {
          ...
          fetch('https://api.resend.com/emails', { method:'POST', headers:{'Content-Type':'application/json','Authorization':'Bearer '+process.env.RESEND_API_KEY}, body: JSON.stringify({ from:'SKY TEAM <onboarding@resend.dev>', to:[socioEmail], subject:'🔔 Nueva cita: '+booking.nombre+' · '+fechaLabel, html }) });
        }
      }
