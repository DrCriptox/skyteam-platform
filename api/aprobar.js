...
export default async function handler(req, res) {
  ...
  try {
    ...
    await fetch(SUPABASE_URL + '/rest/v1/users', {
      ...
      body: JSON.stringify({ username, email: sol.email || null, name: sol.name || null, sponsor: sol.sponsor || null, ref: sol.ref || username, password })
    });
      ...
      try {
        ...
        if (e1.ok) {
          ...
          fetch('https://api.resend.com/emails', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + process.env.RESEND_API_KEY },
            body: JSON.stringify({ from: FROM, to: [sol.email], subject: '🔗 Tu link de duplicación personalizado — SKY TEAM', html: html2 })
          });
        }
