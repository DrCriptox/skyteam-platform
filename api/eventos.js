// api/eventos.js - Sky TV: Events API + Zoom SDK Signature + Push
...
const SB_H = { 'Content-Type': 'application/json', apikey: SUPABASE_SERVICE_KEY, Authorization: 'Bearer ' + SUPABASE_SERVICE_KEY, Prefer: 'return=representation' };
...
async function broadcastPush(title, body, url) {
  ...
  try {
    ...
    const r = await fetch(SUPABASE_URL + '/rest/v1/push_subscriptions?select=*', { headers: SB_H });
    const subs = await r.json();
    if (!subs || !subs.length) return;
    const payload = JSON.stringify({ title, body, url: url || '/', tag: 'skyteam-event-' + Date.now(), data: { url: url || '/' } });
    for (const sub of subs) {
      try { await webpush.sendNotification(sub.subscription, payload); } catch(e) {
        if (e.statusCode === 410 || e.statusCode === 404) {
          try { await fetch(SUPABASE_URL + '/rest/v1/push_subscriptions?endpoint=eq.' + encodeURIComponent(sub.endpoint), { method: 'DELETE', headers: SB_H }); } catch(x){}
        }
      }
    }
