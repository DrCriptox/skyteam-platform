...
async function handleTriggers(req, res) {
  ...
  try {
    ...
    const closingProspects = await sb(
      ...
      '&fecha_cierre_estimada=gte.' + today + '&fecha_cierre_estimada=lte.' + sevenDaysAhead +
      '&etapa=not.in.(cerrado_ganado,cerrado_perdido)' +
      '&limit=20'
    );
...
function waGetActiveWindow(now) {
  // Legacy: compute Colombia time (para backwards compat si alguien llama sin TZ)
  return waGetActiveWindowForLead(now, 'America/Bogota');
}

// v2.1: Compute active window in the LEAD's timezone
function waGetActiveWindowForLead(now, leadTz) {
  var tz = leadTz || 'America/Bogota';
  try {
    ...
    // Fallback: Colombia time (original logic)
...
// v2.1: Detecta la ventana DOMINANTE del lead (morning/lunch/evening) basado en
// response_pattern_hours (horas UTC). Si el lead tiene >=3 respuestas y una
// ventana concentra >=40% de ellas, esa es su ventana preferida. Si no, null.
function waGetDominantBucket(responseHoursUtc, leadTz) {
  if (!Array.isArray(responseHoursUtc) || responseHoursUtc.length < 3) return null;
  const tz = leadTz || 'America/Bogota';
  const buckets = { morning: 0, lunch: 0, evening: 0, other: 0 };
  for (const utcHour of responseHoursUtc) {
    try {
      const d = new Date();
      ...
      let lh = 0;
      parts.forEach(function(p) { if (p.type === 'hour') lh = parseInt(p.value); });
      if (lh === 24) lh = 0;
      const minOfDay = lh * 60;
      // Ventanas relajadas con +/- 60 min para capturar el contexto
      if (minOfDay >= WA_FU_WINDOWS.morning.start - 60 && minOfDay <= WA_FU_WINDOWS.morning.end + 60) buckets.morning++;
      else if (minOfDay >= WA_FU_WINDOWS.lunch.start - 60 && minOfDay <= WA_FU_WINDOWS.lunch.end + 60) buckets.lunch++;
      else if (minOfDay >= WA_FU_WINDOWS.evening.start - 60 && minOfDay <= WA_FU_WINDOWS.evening.end + 60) buckets.evening++;
      else buckets.other++;
    } catch (e) { buckets.other++; }
  }
  const total = buckets.morning + buckets.lunch + buckets.evening + buckets.other;
  const max = Math.max(buckets.morning, buckets.lunch, buckets.evening);
  if (total === 0 || max / total < 0.4) return null;
  if (buckets.morning === max) return 'morning';
  if (buckets.lunch === max) return 'lunch';
  if (buckets.evening === max) return 'evening';
  return null;
}
...
async function runWhatsAppFollowupsV2(now, cHour, todayKey, results) {
  // v2.1: ya NO skipeamos el cron entero por "Colombia fuera de ventana".
  // Cada lead se evalua en su propia TZ.

  const oneDayAgo = new Date(now.getTime() - 24 * 3600000).toISOString();
  const twoHoursAgo = new Date(now.getTime() - 2 * 3600000).toISOString();

  const waLeads = await sb(
    'wa_leads?select=phone,name,followup_stage,last_message_at,last_followup_sent_at,followup_variant,context_summary,objections,etapa,temperatura,bot_username,timezone,response_pattern_hours' +
  ...
  for (const lead of waLeads) {
    // v2.1: compute lead's local active window (usando su TZ, fallback a Bogota)
    const leadTz = lead.timezone || 'America/Bogota';
    const activeWindow = waGetActiveWindowForLead(now, leadTz);
    if (!activeWindow) continue; // fuera de ventana LOCAL del lead
    ...
    // v2.1: si el lead tiene patron de respuesta claro, OVERRIDE el target del stage.
    // Si no hay patron (menos de 3 respuestas o disperso), usa el target original.
    const dominantBucket = waGetDominantBucket(lead.response_pattern_hours, leadTz);
    const effectiveTarget = dominantBucket || stageCfg.target;

    // Stage 1 tiene target=null (ASAP cualquier ventana). Los demas requieren match.
    if (effectiveTarget && effectiveTarget !== activeWindow) continue;
    ...
    results.triggers.push({
      type: 'wa_fu_v2_s' + nextStage,
      phone: lead.phone.substring(0, 6) + '...',
      window: activeWindow,
      tz: leadTz,
      dominant: dominantBucket || null,
      ai: stageCfg.useAI && !variantUsed
    });
    results.sent++;
  }
}
