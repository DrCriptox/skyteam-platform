...
export default async function handler(req, res) {
 ...
 + Number(p.valor_estimado).toLocaleString() + ')' : '';
        const fc = new Date(p.fecha_cierre_estimada);
        const diasRestantes = Math.ceil((fc.getTime() - now.getTime()) / 86400000);
        let title, body;
        if (diasRestantes <= 0) { title = 'Cierra HOY'; body = p.nombre + valor + ' cierra HOY. Llamada de confirmacion ahora.'; }
        else if (diasRestantes === 1) { title = 'Cierra manana'; body = p.nombre + valor + ' cierra manana. Confirma disponibilidad y agenda la llamada final.'; }
        else if (diasRestantes === 2) { title = 'Cierre en 2 dias'; body = p.nombre + valor + ' cierra en 2 dias. Envia mensaje de valor antes de cerrar.'; }
        else if (diasRestantes === 3) { title = 'Cierre en 3 dias'; body = p.nombre + valor + ' cierra en 3 dias. Resuelve dudas pendientes y muestra caso de exito.'; }
        else if (diasRestantes <= 5) { title = 'Cierre en ' + diasRestantes + ' dias'; body = p.nombre + valor + ' cierra en ' + diasRestantes + ' dias. Manten el contacto con valor, no presiones.'; }
        else { title = 'Cierre en ' + diasRestantes + ' dias'; body = p.nombre + valor + ' cierra en ' + diasRestantes + ' dias. Construye confianza esta semana.'; }
        const r = await pushToUser(p.username, title, body, '/?nav=prospectos', 'skyteam-closing-' + p.id + '-' + today);
        results.triggers.push({ type: 'closing_plan', user: p.username, prospect: p.nombre, dias: diasRestantes, sent: r.sent });
        results.sent += r.sent
      }
    }
