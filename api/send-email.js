export default async function handler(req, res) {
  ...
  try {
    ...
    const senderLabels = {
      'lideres@skyteam.global':  'SKY TEAM Líderes',
    ...
    const htmlBody = customHtml || `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#030c1f;color:#F0EDE6;padding:32px;border-radius:16px;">
        ...
        <p style="color:rgba(255,255,255,0.5);font-size:12px;margin-top:24px;text-align:center;">Enviado por ${fromLabel} · <a href="https://skyteam.global" style="color:#1CE8FF;">skyteam.global</a></p>
      </div>
    `;

    const emailSubject = subject || ('🎉 ¡Tu acceso a SKY SYSTEM está activo, ' + nombre + '!');

    const r = await fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + process.env.RESEND_API_KEY },
      body: JSON.stringify({ from: fromField, to: [to], subject: emailSubject, html: htmlBody })
    });
