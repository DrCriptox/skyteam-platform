...
// --- 5. cambiarRango: now handled entirely by index.html (PATCH + reload) ---
// REMOVED: patch-v3 no longer overrides cambiarRango.
// The definitive version lives in index.html and does:
//   1. PATCH /api/update-user with new rank
//   2. loadUsersFromServer() to refresh USERS from DB
...
function renderAdminAgendaDiaria(el, todayLabel, timeStr) {
  lbApi('dailyTop', {}).then(function(data) {
    ...
    html += '<button onclick="_adminAgendaView=\'diaria\';renderAdminAgenda();" style="padding:6px 14px;border-radius:8px;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:#fff;font-size:12px;font-weight:700;cursor:pointer;">Diaria</button>';
...
function renderAdminAgendaSemanal(el, todayLabel, timeStr) {
  lbApi('weeklyTop', {}).then(function(data) {
    ...
    html += '<button onclick="_adminAgendaView=\'semanal\';renderAdminAgenda();" style="padding:6px 14px;border-radius:8px;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:#fff;font-size:12px;font-weight:700;cursor:pointer;">Semanal</button>';
    html += '</div></div>';
